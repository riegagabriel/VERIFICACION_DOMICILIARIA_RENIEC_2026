...
with tab2:
    ...
    if not data_dept.empty:
        ...
        fig_dept.update_layout(
            ...
            xaxis_title='Fecha', yaxis_title='Ciudadanos Verificados',
            hovermode='x unified',
            legend=dict(
    ...
    if not data_dist.empty:
        ...
        depto_sel          = st.selectbox("Seleccionar departamento", departamentos_dist, key="depto_dist_graf")

        df_dist_sel  = data_dist[data_dist['DEPARTAMENTO'] == depto_sel]
        df_total_dep = df_dist_sel.groupby('fecha')['ciudadanos_verificados'].sum().reset_index()
...
with tab3:
    ...
    if data_enc.empty:
        ...
        with col2:
            dists_enc    = sorted(data_enc[data_enc['REG'] == dep_enc_sel]['DIST'].dropna().unique())
